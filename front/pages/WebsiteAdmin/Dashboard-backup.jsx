...
const WebsiteAdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      console.log('🔄 Auto-refreshing deletion requests data...');
      fetchPendingApprovals();
      
      // If we're on the deletion records page, refresh all data
      if (location.pathname.includes('/deletion-records')) {
  ...
  // Force refresh when component first mounts or route changes to deletion records
  useEffect(() => {
    if (location.pathname.includes('/deletion-records')) {
      console.log('🔄 Force refreshing deletion records page data...');
      fetchAllRequestsHistory();
    }
  }, [location.pathname]);
  ...
  // Load analytics and other initial data
  const loadInitialData = async () => {
    ...
    try {
      await Promise.all([
        ...
        fetchAllRequestsHistory() // Load deletion history early
      ]);
    } catch (error) {
  ...
  // Load pending approvals and available approvers - now handled in loadInitialData
  useEffect(() => {
    fetchAvailableApprovers();
  }, []);
  ...
  const handleDeleteCompany = async (id, name) => {
    ...
    // Fetch approvers and auto-select first one
    try {
      await fetchAvailableApprovers();
      // Wait a bit for state to update, then set default selection
      setTimeout(() => {
        const response = websiteAdminService.getAvailableApprovers().then(res => {
          const approvers = Array.isArray(res?.data) ? res.data : res?.data || [];
          if (approvers.length > 0) {
            setSelectedApproverId(String(approvers[0].id));
          }
        });
      }, 200);
    } catch (error) {
  ...
  const handleDeletionRequestSubmit = async () => {
    ...
    try {
      ...
      // Immediately add the new request to pending approvals for instant UI update
      const newRequest = {
        request_id: response.data?.request_id || Date.now(),
        company_id: selectedCompanyForDeletion.id,
        company_name: selectedCompanyForDeletion.name,
        requested_by_name: currentUser?.name || 'Current User',
      ...
      // Refresh pending approvals to get server data (but UI is already updated)
      setTimeout(() => {
        fetchPendingApprovals();
      }, 1000);
  ...
  const fetchPendingApprovals = async () => {
    try {
      ...
      console.log('📋 Current pending approvals state will be:', pendingData.length, 'items');
    } catch (error) {
  ...
  const fetchCompletedRequests = async () => {
    try {
      ...
      console.log('✅ Completed requests response:', response);
  ...
  const fetchAllRequestsHistory = async () => {
    try {
      console.log('🔄 Fetching ALL deletion requests history...');
      setLoadingHistory(true);
      ...
      try {
        // First try the specific history endpoint
        console.log('🔄 Trying getAllDeletionRequestsHistory endpoint...');
        const historyResponse = await websiteAdminService.getAllDeletionRequestsHistory();
        console.log('✅ History API response:', historyResponse);
        allRequests = Array.isArray(historyResponse?.data) ? historyResponse.data : historyResponse?.data || [];
       } catch (historyError) {
        console.warn('⚠️ History endpoint failed, trying individual endpoints:', historyError);
        
        // Fallback: fetch pending and completed separately
        try {
          console.log('🔄 Trying separate pending and completed endpoints...');
          const [pendingResponse, completedResponse] = await Promise.all([
            websiteAdminService.getPendingDeletionRequests(),
            websiteAdminService.getCompletedDeletionRequests()
          ]);
          
          console.log('✅ Pending response:', pendingResponse);
          console.log('✅ Completed response:', completedResponse);
          
          const pendingData = Array.isArray(pendingResponse?.data) ? pendingResponse.data : pendingResponse?.data || [];
          const completedData = Array.isArray(completedResponse?.data) ? completedResponse.data : completedResponse?.data || [];
          
          allRequests = [...pendingData, ...completedData];
          console.log('✅ Combined data from separate endpoints:', allRequests);
        } catch (fallbackError) {
          console.error('❌ Both history methods failed:', fallbackError);
          allRequests = [];
        }
      }
      
      console.log('📊 Final all requests history:', allRequests);
      ...
      console.log('📋 Separated data - Pending:', pending.length, 'Completed:', completed.length);
  ...
  const handleApprovalAction = async () => {
    ...
    try {
      ...
      if (approvalAction === 'approve') {
        // If approved, remove the company from companies list
      ...
      // Remove from pending approvals and add to completed requests
      setPendingApprovals(prev => prev.filter(req => req.request_id !== selectedApproval.request_id));
      
      const completedRequest = {
        ...selectedApproval,
        status: approvalAction === 'approve' ? 'APPROVED' : 'REJECTED',
        processed_at: new Date().toISOString(),
        approved_by_name: currentUser?.name || 'Current User',
        processed_by_name: currentUser?.name || 'Current User',
        rejection_reason: approvalAction === 'reject' ? approvalReason.trim() : undefined,
        approval_reason: approvalAction === 'approve' ? approvalReason.trim() : undefined
      };
      ...
      // Refresh data from server (but UI is already updated)
      setTimeout(() => {
        fetchPendingApprovals();
        fetchCompletedRequests();
        if (activeTab === 'all') {
      ...
      >
        ...
        <div className="approval-info" style={{ flex: 1 }}>
          ...
          {(!isPending) && (
            <>
              <div style={{ color: '#6b7280', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                👨‍💼 <strong>Processed by:</strong> {request.approved_by_name || request.processed_by_name || 'System'}
              </div>
              <div style={{ color: '#6b7280', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
          ...
          {isApproved && request.approval_reason && (
            <div style={{
              color: '#065f46',
              fontSize: '0.875rem',
              marginTop: '0.5rem',
              ...
              💬 <strong>Approval Notes:</strong> {request.approval_reason}
            </div>
          )}
        </div>
          ...
          <Routes>
            <Route path="/dashboard" element={
              <>
                ...
                {/* Company Deletion Requests Management - Always show for authorized users */}
                {(currentUserCanCreate || currentUser?.email === 'omvataliya23@gmail.com') && (
                  <div className="card" style={{
                    ...
                    {pendingApprovals.length > 0 && (
                      <div className="approvals-list" style={{ marginBottom: '1.5rem' }}>
                        <h3 style={{ 
                          fontSize: '1.125rem', 
                          fontWeight: '600', 
                          marginBottom: '1rem',
                    ...
                    {pendingApprovals.length === 0 && (
                      <div style={{ 
                        ...
                        <h3 style={{ marginBottom: '0.5rem', fontWeight: '500', color: '#374151' }}>No Pending Deletion Requests</h3>
                        <p style={{ color: '#6b7280' }}>All company deletion requests have been processed.</p>
                      </div>
                    )}
                          ...
                          {activeTab === 'history' && (
                            <div className="history-requests">
                              {completedRequests.length === 0 ? (
                                ...
                                completedRequests.map((request, index) => renderHistoryCard(request, index))
                              )}
                            </div>
                          )}
                        </>
                      ...
                      <table className="data-table" style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0', textAlign: 'left' }}>
                        <thead>
                          <tr style={{ backgroundColor: '#f9fafb' }}>
                            <th style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb', fontWeight: '500', color: '#6b7280' }}>Company</th>
                        ...
                        <tbody>
                          {companies.map((company, index) => {
                            // Check if there's a pending deletion request for this company
                              ...
                              <tr key={company.id || index} style={{ backgroundColor: index % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                                <td style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                    {company.logoUrl && (
                                      <img 
                                        src={company.logoUrl.startsWith('http') ? company.logoUrl : `${env.apiHost}${company.logoUrl}`}
                                    ...
                                    <div>
                                      ...
                                      <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                                        {company.projectName || 'No project name'}
                                      </div>
                                    </div>
                                  </div>
                                </td>
                                <td style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb' }}>{company.domain || "-"}</td>
                                <td style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb' }}>{company.industry || "-"}</td>
                                <td style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb' }}>{company.size || "-"}</td>
                                <td style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb', color: '#6b7280' }}>
                                  {company.createdAt ? 
                                    new Date(company.createdAt).toLocaleString(undefined, { 
                                      year: 'numeric', 
                                      month: 'short', 
                                      day: 'numeric',
                                      hour: '2-digit',
                                  ...
                                  completedRequests.map((request, index) => (
                                    <div key={request.request_id || index} className="history-item" style={{
                                      ...
                                      <div className="request-info" style={{ flex: 1 }}>
                                        ...
                                        <div style={{ color: '#6b7280', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                                          👨‍💼 <strong>Processed by:</strong> {request.approved_by_name || 'System'}
                                        </div>
                                        <div style={{ color: '#6b7280', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                                          📅 <strong>Processed on:</strong> {new Date(request.processed_at || request.updated_at).toLocaleString()}
                                        </div>
                              ...
                              return (
                                <tr key={`${record.status}-${record.request_id || index}`} style={{ backgroundColor: index % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                                  ...
                                  <td style={{ padding: '1rem' }}>
                                    <div>
                                      <div style={{ fontWeight: '500' }}>{record.requested_by_name}</div>
                                      <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>{record.requested_by_email}</div>
                                    </div>
                                  </td>
                                  ...
                                  <td style={{ padding: '1rem' }}>
                                    {isPending ? (
                                      ...
                                      <div style={{ fontWeight: '500' }}>
                                        {record.approved_by_name || record.processed_by_name || 'System'}
                                      </div>
                                    )}
                                  </td>
                                  <td style={{ padding: '1rem', color: '#6b7280', fontSize: '0.875rem' }}>
