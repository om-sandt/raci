...
const EditCompany = () => {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        ...
        if (data.logoUrl) {
          const logoUrl = `${env.apiBaseUrl}${data.logoUrl}`;
        ...
        if (data.projectLogo) {
          const projectLogoUrl = `${env.apiBaseUrl}${data.projectLogo}`;
          setOriginalProjectLogo(projectLogoUrl);
          setProjectLogoPreview(projectLogoUrl);
        } else {
          setOriginalProjectLogo('');
  ...
  const handleLogoChange = (e) => {
    ...
    if (file) {
      ...
      // If no file selected, reset to original logo or clear preview
      setLogo(null);
      setLogoPreview(originalLogo || '');
    }
  };
  ...
  const handleProjectLogoChange = (e) => {
    ...
    if (file) {
      // Validate file type
      if (!file.type.startsWith('image/')) {
        setError('Please select a valid image file for project logo');
        return;
      }
      
      // Validate file size (limit to 5MB)
      if (file.size > 5 * 1024 * 1024) {
        setError('Project logo file size must be less than 5MB');
        return;
      }
      ...
      reader.onerror = () => {
        setError('Error reading the project logo file');
        setProjectLogo(null);
        setProjectLogoPreview(originalProjectLogo || '');
      };
      reader.readAsDataURL(file);
    } else {
      // If no file selected, reset to original project logo or clear preview
      setProjectLogo(null);
      setProjectLogoPreview(originalProjectLogo || '');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Updating company with data:', {
        ...
        hasProjectLogo: projectLogo instanceof File,
        logoFileName: logo?.name,
        projectLogoFileName: projectLogo?.name
      });
          ...
          <div className="form-group full-width">
            ...
            {projectLogoPreview && (
              <div className="logo-preview" style={{ marginTop: '15px', maxWidth: '200px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                  <p style={{ fontSize: '0.8rem', color: '#6b7280', margin: 0 }}>
                    {projectLogo ? 'New project logo preview' : 'Current project logo'}
                  </p>
                  {projectLogo && (
                    <button 
                      type="button"
